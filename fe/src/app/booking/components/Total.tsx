...
const Total = ({ mode, bookingData, selectedTrainingSession, equipmentItems, foodDrinkItems }: TotalProps) => {
    ...
    const bookingPrice = mode === 'court' ? calculateCourtPrice() : calculateTrainingPrice();
    ...
    const isBookingSelectionValid =
        (mode === 'court' && bookingData.selectedCourt !== null && bookingData.duration && bookingData.duration > 0) ||
    ...
    const handlePaymentSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (mode === 'court' && (!bookingData.date || !bookingData.startTime || !bookingData.endTime || !bookingData.duration || !bookingData.selectedCourt)) return;
        if (mode === 'training' && !selectedTrainingSession) return;


        setIsProcessing(true);
        console.log(`Processing payment for ${grandTotal.toLocaleString("vi-VN")} VND via ${selectedPaymentMethod} (Mode: ${mode})`);
        ...
        if (mode === 'court' && bookingData.selectedCourt && bookingData.date && bookingData.startTime && bookingData.endTime && bookingData.duration) {
             const isPremiumCourt = bookingData.selectedCourt === 1 || bookingData.selectedCourt === 4;
            ...
            console.error("Error creating confirmation details: Invalid state.");
        ...
        <>
            ...
            <section className={styles.totalSection}>
                 ...
                 <div className={styles.totalContainer}>
                     ...
                     {mode === 'court' && bookingPrice > 0 && bookingData.selectedCourt && (
                         <div className={styles.summaryItem}>
                            <span>Court {bookingData.selectedCourt} ({bookingData.duration || 0}h):</span>
                            <span className={styles.summaryPrice}>{bookingPrice.toLocaleString("vi-VN")} VND</span>
                        </div>
                     )}
                     {mode === 'training' && selectedTrainingSession && bookingPrice > 0 && (
                         <div className={styles.summaryItem}>
                            <span>Training: {selectedTrainingSession.name}</span>
                            <span className={styles.summaryPrice}>{bookingPrice.toLocaleString("vi-VN")} VND</span>
                         </div>
                     )}
                ...
                <div className={styles.paymentMethodSelector}>
                 ...
                 <form onSubmit={handlePaymentSubmit} className={styles.paymentFormContainer}>
                    {/* Conditional Info (No change needed) */}
                     {selectedPaymentMethod === 'momo' && ( <div className={styles.digitalWalletInfo}><p>You will be prompted...</p></div> )}
                     {selectedPaymentMethod === 'zalopay' && ( <div className={styles.digitalWalletInfo}><p>You will be prompted...</p></div> )}
                     {selectedPaymentMethod === 'bank-transfer' && ( <div className={styles.digitalWalletInfo}><p>Details for bank transfer...</p></div> )}
                    ...
                    {isPaymentDisabled && grandTotal > 0 && !isProcessing && !selectedPaymentMethod && (
                     ...
                     {isPaymentDisabled && grandTotal > 0 && !isProcessing && !isBookingSelectionValid && (
