...
const availableSessions: TrainingSession[] = [
    {
        ...
        level: "Beginner",
        description: "Master the basics: grip, footwork, serve, and clear shots. Perfect for new players.",
        coach: "Ken Nguyen",
    ...
    },
    {
        ...
        level: "Intermediate",
        description: "Refine your strokes, improve net play, drops, and smashes. Learn basic game strategies.",
        coach: "Nguyen Ken",
    ...
    },
    {
        ...
        level: "Advanced",
        description: "High-intensity drills, competitive match play, advanced footwork, and tactical analysis.",
        coach: "Dien Nguyen",
...
const TrainingSessionForm = ({ selectedSession, onSessionSelect }: TrainingSessionFormProps) => {
    return (
            ...
            {availableSessions.length > 0 ? (
                <div className={styles.trainingGrid}>
                    {availableSessions.map(session => (
                        <div
                            key={session.id}
                            className={`${styles.trainingCard} ${selectedSession?.id === session.id ? styles.selected : ''}`}
                            onClick={() => onSessionSelect(session)}
                            role="button" // Improve accessibility
                            tabIndex={0} // Make it focusable
                            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onSessionSelect(session); }} // Keyboard activation
                        >
                            <div className={styles.trainingInfo}>
                                <h3>{session.name} <span className={styles.levelBadge}>{session.level}</span></h3>
                                <p><strong>Coach:</strong> {session.coach}</p>
                                <p><strong>Schedule:</strong> {session.schedule}</p>
                                <p><strong>Assigned Court:</strong> {session.courtAssigned}</p>
                                <p className={styles.trainingDescription}>{session.description}</p>
                 ...
                 <p className={styles.noSessions}>No training sessions currently available.</p>
            )}
        </section>
    );
};
